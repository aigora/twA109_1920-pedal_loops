...
void draw()
{
  if(inicio == 1) //menú
  {
    titulo(); // función para mostrar el menú principal
  }
  else
  { 
  ...
  puntuaciones(); // calcular la puntuación obtenido y los castigos
  m_bola(); // controlar el movimiento de la bola
...
void m_bola() // controlar el movimiento de la bola
{
...
int m_bolay()//movimiento en el eje y
{
  ...
  if ((y > height-R) || (y < R))// si choca contra el muro
   ...
   if((by+R>=py)&&(by+R<py+R/2))//si choca contra la plataforma
    if(((bx>=px) && (bx<=px+L))||((bx+R>px)&&(bx<=px))||((bx-R<px+L)&&(bx>=px+L)))
    if(estado==1)//se rebota
      {
      if(dy<0)
       dy = dy+0;// se rebota
